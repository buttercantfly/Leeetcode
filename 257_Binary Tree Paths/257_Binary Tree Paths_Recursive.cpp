...
note:
    Recursive:
    start at root, end at leaf
    => 這種樹狀top down的方式可以用void去跑
...
public:
    ...
    void path(TreeNode* node, string route, vector<string>& ret) {
        if (!node) return;
        if (!node->left && !node->right) ret.push_back(route + to_string(node->val));
        else {
            if (node->left)  path(node->left, route + to_string(node->val) + "->", ret);
            if (node->right) path(node->right, route + to_string(node->val) + "->", ret);   
        }
    }
};
