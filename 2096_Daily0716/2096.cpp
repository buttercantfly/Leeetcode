...
public:
    ...
    bool getPath(TreeNode* cur, int destVal, string& path) {
        if (cur->val == destVal) return true;
        if(!cur->left && !cur->right) return false;

        if (cur->left && getPath(cur->left, destVal, path)) {
            path += "L";
            return true;
        }

        if (cur->right && getPath(cur->right, destVal, path)) {
            path += "R";
            return true;
        }
